...
const Topper = ({
  ...
  return (
    <div className="fixed inset-x-0 clamp-[bottom,4,8,@sm,@lg] flex flex-col items-center pointer-events-none w-full z-50 gap-3">
      {/* indicator shown above the main topper when editing a pack */}
      {typeof editPackIndex === "number" && editPackIndex >= 0 && (
        <div className="pointer-events-auto max-w-xl w-full clamp-[py,1.125rem,1.375rem,@sm,lg] clamp-[px,2,4,@sm,@lg] bg-[#FFE6DD] text-[#F95B1E] rounded-[8px] between">
          <p className="clamp-[text,xs,sm,@sm,@lg] leading-5 font-medium start">
            <Icon icon="info" size={15.416666984558105} className="mr-1" />
